<?php
/**
* Telegram Bot example for dovedormireinsicilia_bot
* @author Francesco Piero Paolicelli @piersoft with @cirospataro
*/
 ...
 function shell($telegram,$text,$chat_id,$user_id,$location,$reply_to_msg)
{
	...
	if ($text == "/start" || $text == "©️ Informazioni") {
		...
		file_put_contents('/usr/www/piersoft/dovedormireinsiciliabot/db/telegram.log', $log, FILE_APPEND | LOCK_EX);
...
if ($filter==$text ){
	...
	$homepage = "Nome: <b>".$parsed_json->{'features'}[$i]->{'attributes'}->{'AccomDesc'}."</b>\n";
	$homepage .= "Tipologia: <b>".$parsed_json->{'features'}[$i]->{'attributes'}->{'Tipology'}."</b>\n";
		if ($parsed_json->{'features'}[$i]->{'attributes'}->{'Classification'} !=null) $homepage .= "Classificazione: ".$parsed_json->{'features'}[$i]->{'attributes'}->{'Classification'}."\n";
		$homepage .= "Località: ".$parsed_json->{'features'}[$i]->{'attributes'}->{'GeogAreaName'}."\n";
